...
class AMT(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        # videos, video_lens = batch.sig2
        # videos, video_lens = videos.to(self.device), video_lens.to(self.device)

        videos = torch.zeros_like(wavs).to(self.device)

        # feature fusion module
        feats = self.modules.fusion(wavs, videos)
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            if cur_utter == all_utter:
                ...
                wav_path = batch.audio
                pred_save_folder = os.path.dirname(wav_path[0])
                if add_noise:
                    pred_save_path = os.path.join(pred_save_folder,  noise_type, f"SNR_{snr_db}dB_audio_pred.npy")
                else:
                    pred_save_path = os.path.join(pred_save_folder, "clean_audio_pred.npy")
                np.save(pred_save_path, est_result_np)
    ...
    def on_fit_start(self):
        ...
        if self.hparams.pretrain:
            logger.info(f"Load classifier model weights from the folder: {self.hparams.pretrain_folder}")
            self.modules.head.load_state_dict(torch.load(os.path.join(self.hparams.pretrain_folder, "model.pt")))
        else:
...
def dataio_prepare(hparams):
    ...
    sample_rate1 = hparams["audio_sample_rate"]
    sample_rate2 = hparams["video_sample_rate"]
    frame_rate = hparams["frame_rate"]
    snr_db = hparams["snr_db"]
    noise_type = hparams["noise_type"]

    # 2. Define audio and video pipeline:
    @sb.utils.data_pipeline.takes("audio", "video", "utter_id", "utter_num")
    @sb.utils.data_pipeline.provides("sig1")
    def audio_visual_pipeline(audio, video, utter_id, utter_num):
        if hparams["add_noise"]:
            folder = os.path.dirname(audio)
            audio_noise = os.path.join(folder, noise_type, f"SNR_{snr_db}dB_feats.pt")
        else:
            audio_noise = audio
        # load audio and video signal
        sig1 = torch.load(audio_noise)  # audio: (frame1, 1024)
        # sig2 = torch.load(video)  # video: (frame2, 1024)
        # split the utterances
        utter_id = int(utter_id)
        utter_num = int(utter_num)
        if utter_id == utter_num:
            # audio
            num_sample = (utter_id - 1) * sample_rate1 * dur_threshold
            num_sample = round(num_sample)
            sig1 = sig1[num_sample:]
            # # video
            # num_sample = (utter_id - 1) * sample_rate2 * dur_threshold
            # num_sample = round(num_sample)
            # sig2 = sig2[num_sample:]
        else:
            # audio
            num_sample1 = (utter_id - 1) * sample_rate1 * dur_threshold
            num_sample2 = utter_id * sample_rate1 * dur_threshold
            num_sample1 = round(num_sample1)
            num_sample2 = round(num_sample2)
            sig1 = sig1[num_sample1:num_sample2]
            # # video
            # num_sample1 = (utter_id - 1) * sample_rate2 * dur_threshold
            # num_sample2 = utter_id * sample_rate2 * dur_threshold
            # num_sample1 = round(num_sample1)
            # num_sample2 = round(num_sample2)
            # sig2 = sig2[num_sample1:num_sample2]
        # # align the audio and video features
        # frame1, frame2 = sig1.shape[0], sig2.shape[0]
        # if frame1 < frame2:
        #     sig2 = sig2[:frame1]
        # elif frame1 > frame2:
        #     sig2 = torch.cat([sig2, torch.zeros(frame1 - frame2, 1024)], dim=0)
        return sig1#, sig2

    sb.dataio.dataset.add_dynamic_item(datasets, audio_visual_pipeline)
    ...
    sb.dataio.dataset.set_output_keys(
        datasets,
        ["id", "audio", "sig1", "anno", "cur_utter", "all_utter", "ref_intervals", "ref_pitchs"],
    )
