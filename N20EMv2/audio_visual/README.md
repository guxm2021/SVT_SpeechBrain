# Audio-Visual Singing Voice Transcription with N20EMv2 dataset
This sub-project contains recipes for trianing audio-visual SVT system using N20EMv2 dataset.
...
## How to run
1. Firstly, we train our audio-only SVT system (check `N20EMv2/audio_only/README.md`) and video-only SVT system (check `N20EMv2/video_only/README.md`). Make sure the model are well saved in order to extract features as follows.

2. Secondly, we fix the gradients of audio encoder and video encoder. Practically, we extract the acoustic features from audio-only SVT system. To do so, run following commands:
```
...
```
Compared to the run script, we enable four extra options. `--add_noise` refers to noisy input or clean input, `--noise_type` refers to the type of noise, including `accomp`, `babble`, `white`, `natural`, while `--snr_db` refers to the noise level, including `-10`, `-5`, `0`, `5`, `10` dB. Finally, `--save_model==True` means that we will save the SSL model and SVT classifier to the folder `N20EMv2/save_model` 

Then we extract the visual features from video-only SVT system. To do so, run following commands:
```
...
4. Run the second training stage of audio-visual SVT system, run:
```
...
```
We also provide counterpart audio-only SVT system trained via two stages, run:
```
