...
class ResEncoder(nn.Module):
    def __init__(self, relu_type, weights):
        ...
        frontend_relu = nn.PReLU(num_parameters=self.frontend_nout) if relu_type == 'prelu' else nn.ReLU(inplace=True)
...
class SubModel(nn.Module):
    def __init__(self, input_dim=512, embed_dim=1024, relu_type='prelu', weights=None):
        super().__init__()
        self.resnet = ResEncoder(relu_type=relu_type, weights=weights)
