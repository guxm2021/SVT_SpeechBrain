#!/usr/bin/env/python3
"""Recipe for training a encoder-based AMT system with MIR-ST500 dataset
The system employs encoder 2.0 as its encoder. 
...
class AMT(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        return onset_logits, offset_logits, pitch_octave_logits, pitch_class_logits, video_lens, feats

    def compute_objectives(self, predictions, batch, stage):
        """Computes the loss (CTC+NLL) given predictions and targets."""
        # predictions
        onset_logits, offset_logits, pitch_octave_logits, pitch_class_logits, video_lens, feats = predictions
        ...
        if stage != sb.Stage.TRAIN:
            ...
            if cur_utter == all_utter:
                ...
                feat_save_folder = os.path.dirname(video_path[0])
                song_feats_tensor = torch.cat(self.song_feats, dim=0) # frame-level concatenation
                feat_save_path = os.path.join(feat_save_folder, "noise_data", "video_feats.pt")
...
def dataio_prepare(hparams):
    ...
    sb.dataio.dataset.set_output_keys(
        datasets,
        ["id", "sig", "anno", "cur_utter", "all_utter", "ref_intervals", "ref_pitchs", "video"],
    )
