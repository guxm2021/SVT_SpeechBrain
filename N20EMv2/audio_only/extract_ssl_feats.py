...
class SVT(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        return onset_logits, offset_logits, pitch_octave_logits, pitch_class_logits, wav_lens, feats

    def compute_objectives(self, predictions, batch, stage):
        """Computes the loss (CTC+NLL) given predictions and targets."""
        # predictions
        onset_logits, offset_logits, pitch_octave_logits, pitch_class_logits, wav_lens, feats = predictions
        ...
        if stage != sb.Stage.TRAIN:
            ...
            self.song_feats.append(feats[0].detach().cpu())
            if cur_utter == all_utter:
                ...
                wav_path = batch.wav
                feat_save_folder = os.path.dirname(wav_path[0])
                song_feats_tensor = torch.cat(self.song_feats, dim=0) # frame-level concatenation
                if add_noise:
                    feat_save_path = os.path.join(feat_save_folder,  "noise_data", noise_type, f"SNR_{snr_db}dB_feats.pt")
                else:
                    feat_save_path = os.path.join(feat_save_folder, "noise_data", "clean_feats.pt")
                torch.save(song_feats_tensor, feat_save_path)
    ...
    def on_stage_start(self, stage, epoch):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            self.song_feats = []   # record features of each song
...
def dataio_prepare(hparams):
    ...
    def audio_pipeline(wav, utter_id, utter_num):
        if hparams["add_noise"]:
            ...
            wav_noise = os.path.join(folder, "noise_data", noise_type, f"SNR_{snr_db}dB.wav")
        else:
            wav_noise = wav
        sig = sb.dataio.dataio.read_audio(wav_noise)
    ...
    sb.dataio.dataset.set_output_keys(
        datasets,
        ["id", "wav", "sig", "anno", "cur_utter", "all_utter", "ref_intervals", "ref_pitchs"],
    )
